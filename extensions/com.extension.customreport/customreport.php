 ...
 *
 */
class com_extension_customreport extends CRM_Report_Form {
...
//custom
   ...
   if (array_key_exists('civicrm_activity_duration', $row)) {
        if ($value = $row['civicrm_activity_duration']) {
          $rows[$rowNum]['civicrm_activity_duration'] = $rows[$rowNum]['civicrm_activity_duration'];//$activityStatus[$value];
          $total_duration += $rows[$rowNum]['civicrm_activity_duration'];
          $entryFound = TRUE;
        }
      }
      ...
      $rows[$rowNum+1]['civicrm_activity_status_id'] = "Total Duration";
   $rows[$rowNum+1]['civicrm_activity_duration'] = $total_duration;
  }
}
