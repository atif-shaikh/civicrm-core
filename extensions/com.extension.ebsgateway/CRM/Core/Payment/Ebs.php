...
class CRM_Core_Payment_Ebs extends CRM_Core_Payment {
	...
	function doTransferCheckout(&$params, $component = 'contribute') {
		...
		$securehash = md5($hash);	
		 ...
		 $name = (isset($params['billing_first_name']) && isset($params['billing_last_name']) ) ? $params['billing_first_name'].' '.$params['billing_last_name'] : $params['first_name'].' '.$params['last_name'];
		 $address = isset($params["billing_street_address-5"]) ? $params["billing_street_address-5"] : $params["street_address-1"];
		 $city = isset($params["billing_city-5"]) ? $params["billing_city-5"] : $params["city-1"];
		 $postal_code = isset($params["billing_postal_code-5"]) ? $params["billing_postal_code-5"] : $params["postal_code-1"];
		 $state_id = isset($params["billing_state_province_id-5"]) ? $params["billing_state_province_id-5"] : $params["state_province-1"];
		if(isset($state_id)){
			$stateName = CRM_Core_PseudoConstant::stateProvinceAbbreviation($state_id);
		}
		 $state = $stateName;
		$country_id = isset($params["billing_country_id-5"]) ? $params["billing_country_id-5"] : $params["country-1"];
		if(isset($state_id)){
			$countryName = CRM_Core_PseudoConstant::countryIsoCode($country_id);
		}
		 $country = $countryName;
		 $phone = isset($params["billing_phone-5"]) ? $params["billing_phone-5"] : $params["phone-Primary-1"];
